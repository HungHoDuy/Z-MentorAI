...
def read_local_results() -> dict:
    if not os.path.exists(settings.holland_results_path):
        return {"assessments": {}}
    try:
        with open(settings.holland_results_path, "r", encoding="utf-8") as file:
...
def write_local_results(data: dict) -> None:
    with open(settings.holland_results_path, "w", encoding="utf-8") as file:
...
async def get_latest_holland_assessment(user_id: str) -> Optional[dict]:
    if settings.use_firestore:
        docs = (
            firestore_client.collection(settings.holland_collection_name)
