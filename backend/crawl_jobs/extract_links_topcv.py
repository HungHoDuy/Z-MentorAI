...
from crawl_jobs.crawler_topcv import TopCVCrawler
...
def main():
    ...
    seen_ids = set()
    try:
        print("Fetching existing job IDs from Firestore 'todo_topcv'...")
    ...
    def check_stop_condition():
        ...
        for p in range(1, max_continuous - 1):
            # If 3 consecutive pages are empty, we stop extraction
        ...
        try:
            while not stop_extraction:
                ...
                try:
                    ...
                    print(f"[Worker-{thread_id}] Error fetching page {page} listing: {e}")
                    # Put page back/stop on consecutive failures
                    with page_lock:
                        stop_extraction = True
                    break
                ...
                time.sleep(3.0) # Respectful delay between page fetches
        finally:
            crawler.close()

    # Start 2 worker threads (since headless is False, keep threads low to prevent focus-stealing issues)
    num_workers = 2
    threads = []
    print(f"Starting {num_workers} parallel extraction threads (headless=False)...")
